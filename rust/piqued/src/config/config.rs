...
impl Config {
    ...
    pub async fn load(file: &Option<PathBuf>, working_dir: &PathBuf) -> Result<Self> {
        let mut piqued_config: Config = if let Some(path) = file {
        ...
        if piqued_config.workspace.root.is_none() {
            piqued_config.workspace.root = Some(working_dir.clone());
        }

        if let Some(root) = &piqued_config.workspace.root {
            let root = root.clone();
            let root = root.canonicalize()?;
            piqued_config.workspace.root = Some(root);
        }
